...
\midrule
Upstream, priors on & \textbf{8.5} (8,\,9) & 2 & upstream prompt and planner; \textbf{112 answer-file reads across 10/10 runs} --- payload carries per-cell answers for \textbf{seed 0 only} (156 files; zero for seeds 1--9) \\
...
\rowcolor{soft} Full design, repaired library & \textbf{8.67\,$\pm$\,0.58} (9,\,9,\,8) & 3 & + issue 5f closed, t9 resident mined in \\
\multicolumn{4}{@{}l@{}}{\textit{Full benchmark --- separate protocol, turns=100, 8 suites, 80 cells, 3 N/A:}} \\
Full-bench gen 0 & \textbf{53/77} & 1 & clean-room + instrument; first contact on seven unseen suites \\
\rowcolor{soft} Full-bench gen 1 & \textbf{45/77} & 1 & + grown library; tax measured at $0\rightarrow9.1$ reads/run \\
Full-bench gen 1b & \textbf{34/53} & 1 & 15-entry constitution + index; stopped at 53 of 77 cells by decision --- \textbf{not} a full benchmark \\
\multicolumn{4}{@{}l@{}}{\textit{On the same 53 cells all three ran: gen 0 \textbf{40}, gen 1 \textbf{34}, gen 1b \textbf{34} --- the pre-registered ``no recovery'' branch.}} \\
\bottomrule
\end{tabularx}
\end{table}
...
\section{Generation 7: the repair, and the cell it cost}

Closing the stopping-licence defect and mining the t9 resident session produced
a repaired library. Three further passes: \textbf{9, 9, 8 --- mean
8.67\,$\pm$\,0.58}. Both targeted cells moved, and t5 is now \textbf{6/6}
post-fix across two independent batches.

\begin{table}[htbp]
\centering\small
\begin{tabular}{@{}l c c c@{}}
\toprule
\textbf{Cell} & \textbf{blind} & \textbf{instrument} & \textbf{instrument + repaired library} \\
\midrule
...
\noindent
\textbf{And t1 fell to 0/3.} The immediate suspicion was that the entry revision
...
\begin{caution}[title={Why a licence-assisted solve cannot be lost}]
Scoring uses the environment's flag. A licence-assisted stop is by definition
one where the agent stopped while the flag stayed \emph{silent} --- which
\textbf{already scored as a failure}. The old clause could never manufacture a
counted solve, so withdrawing it cannot remove one. The earlier t1 solves had
the flag fire; they were genuine.
\end{caution}

\noindent
Two of the three failures never read the revised entry, and all three died on
\texttt{reached max\_turns} with no \texttt{finish} --- the opposite of stopping
early. The measured cause is a gradient:

\begin{table}[htbp]
\centering\small
\begin{tabular}{@{}l c c c l@{}}
\toprule
\textbf{run} & \textbf{motion calls} & \textbf{perception calls} & \textbf{ratio} & \\
\midrule
blind, solved      & 9  & 32  & \textbf{0.28} & solved \\
instrument, solved & 9  & 41  & \textbf{0.22} & solved \\
generation 7       & 11 & 72  & 0.15 & failed \\
generation 7       & 12 & 93  & 0.13 & failed \\
generation 7       & 3  & 153 & \textbf{0.02} & failed \\
\bottomrule
\end{tabular}
\end{table}

\noindent
\textbf{Motion stays flat; perception climbs fivefold.} They are not acting
less, they are looking more, and the budget goes to looking --- one run issued
three motions across 167 calls, 44 of them perception. This is the gen-0
perception spiral returning \emph{with a new channel to spiral in}, and the arc
agrees: t1's decline began when the instrument was added, one generation before
the entry was revised. Remedy filed and not built; the method is frozen through
delivery. $n=3$, and t1 was variance-prone historically.
